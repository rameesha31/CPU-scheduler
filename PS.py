...
from first import Process, read_processes_from_dataframe, priority_scheduling_preemptive, evaluate_performance
        ...
        def animate():
            ...
            if tab_isplaying and tab_count < len(frames):
                ...
                if tab_count == len(frames)-1:
                    ...
                    def next():
                        app.destroy()
                        subprocess.run([sys.executable, 'MQS.py'])
                    ...
                    def back():
                        app.destroy()
                        subprocess.run([sys.executable, 'RR.py'])
